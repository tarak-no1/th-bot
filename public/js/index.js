...
var socket = io.connect(ROOT_URL, {'path': '/tata_housing_socket'});
var visitorName = '';
var visitorEmail = '';
var visitorNumber = '';
...
var req = function (){
    $.ajax({
        url : ROOT_URL+'/tata_housing/dummy',
        success : function () {
                online();
            setTimeout(function () {
                req();
...
function showScreen() {
    if (localStorage.getItem("tata_housing_chat_content") != null || localStorage.getItem("tata_housing_chat_content") != undefined) {
        $('#landing-container').hide();
        $('.selekt-conversation-body-parts').show();
        $('.selekt-conversation-footer').show();
        $(localStorage.getItem("tata_housing_chat_content")).appendTo($('.selekt-conversation-parts'));
        ...
        console.log("Chat content null : ", localStorage.getItem("tata_housing_chat_content"));
        $('#landing-container').show();
        $('#visitor_name').val(localStorage.getItem('tata_housing_visitor_name'));
...
$(document.body).on('click', '.control-right', function () {
    ...
    var scrollCar = $(this).parent(".carousal_container").find('.scroll_carousal');
    scrollCar.animate({
...
$(document.body).on('click', '#get_started', function () {
    ...
    if (isValidName) {
        if (rememberMe) {
            ...
            localStorage.setItem("tata_housing_visitor_name", visitorName);
...
socket.on('reconnect', function (data) {
    ...
    var message = {
        device_id: device_id,
        user_name: localStorage.getItem('tata_housing_visitor_name')
    };
    socket.emit('reconnect', JSON.stringify(message));
});
...
socket.on('clear', function () {
    ...
    localStorage.removeItem("tata_housing_chat_content");
    showScreen();
});
...
socket.on('expired', function () {
    ...
    if (localStorage.getItem("remember_me") == "false" || localStorage.getItem("remember_me") == null || localStorage.getItem("remember_me") == undefined) {
        localStorage.removeItem("tata_housing_visitor_name");
    }
    localStorage.removeItem("tata_housing_chat_content");
    location.reload();
});
...
socket.on('carousal_images', function (data) {
    console.log("Carousel with image only", data);
    removeTyping();
    addCarousalImagesOnly(data);
    if (data.gif_status === true) {
        addTyping();
    }
    updateScrollbar();
});
socket.on('google_map', function (data) {
    console.log("Carousel with maps only", data);
    removeTyping();
    addCarousalMapsOnly(data);
    if (data.gif_status === true) {
        addTyping();
    }
    updateScrollbar();
});
socket.on('carousel_subtitle_with_out_options', function (data) {
    console.log("Carousel with carousel_subtitle_with_out_options", data);
    removeTyping();
    addCarousalSubtitleWithoutOptions(data);
    if (data.gif_status === true) {
        addTyping();
    }
    updateScrollbar();
...
$(document.body).on('click', '.carousel_option', function () {
    ...
    }else if (belongs.split(',')[1] === 'enable') {
        addUserMessage(typeCarousel, showMsg, belongs.split(',')[0]);
        $('.carousel_option').each(function(){
            if(this.name.split(',')[1] === 'postback'){
                $(this).prop('disabled', false);
...
function saveChatHistory() {
    localStorage.setItem("tata_housing_chat_content", $('.selekt-conversation-parts').html());
}
...
function addCarousalImage(data) {
    ...
    for (var i in data.options) {
        ...
        for (var x in data.options[i].buttons) {
            ...
            else if (data.options[i].buttons[x].type === 'enable') {
                carousal += '<button name="' + data.belongs + ',' + 'enable' + '" value="' + data.options[i].buttons[x].payload + '" id="single_button_carousal_' + data.options[i].buttons[x] + '" class="carousel_option" style="display: block; font-size: 13px; ">' + data.options[i].buttons[x].title + '</button>';
...
function addCarousalImagesOnly(data) {
    disableChat();
    chatCount++;
    var carousalContainer = $('#main-container-carousal').clone();
    carousalContainer.appendTo($('.selekt-conversation-parts')).show();
    ...
    for (var i in data.options) {
        var carousal = '<div class="carouselala-inner" id="carousal_' + chatCount + '_option_' +
            '" style=" text-align: justify; width:  calc(100% - 20px); height: auto; overflow: hidden; border: 1px solid #295ba2; border-radius: 5px;">' +
            '<div class="image" style="width: 100%; height:100%; max-height: 200px; background: transparent url(' + data.options[i] + ') no-repeat scroll center 0;background-size: cover;"></div>';
        $(carousal).appendTo(carousal_container);
    }
    if (data.options.length == 1) {
        carousalContainer.find('.controls').css('display', 'none')
...
function addCarousalMapsOnly(data) {
    disableChat();
    chatCount++;
    var carousalContainer = $('#main-container-carousal').clone();
    carousalContainer.appendTo($('.selekt-conversation-parts')).show();
    ...
    var carousal_container = carousalContainer.find('.scroll_carousal');
        var carousal = '<div class="carouselala-inner" id="carousal_' + chatCount + '_option_' +
            '" style=" text-align: justify; width:  calc(100% - 20px); height: auto; overflow: hidden; border-radius: 5px;">' +
            '<iframe class="image" frameborder="0" style=" border: 0; width: 100%; height:100%; max-height: 200px;" src="https://www.google.com/maps/embed/v1/place?key=AIzaSyBlncNWaYNJBjp6mfJG_61uT2wN75XE9aI&q='+data.options.lat+','+data.options.lon+'&amp"></iframe>';
        $(carousal).appendTo(carousal_container);
        carousalContainer.find('.controls').css('display', 'none')
    updateScrollbar();
    saveChatHistory();
...
function addCarousalSubtitleWithoutOptions(data) {
    disableChat();
    chatCount++;
    var carousalContainer = $('#main-container-carousal').clone();
    carousalContainer.appendTo($('.selekt-conversation-parts')).show();
    ...
    for (var i in data.options) {
        var carousal = '<div class="carouselala-inner" id="carousal_' + chatCount + '_option_' +
            ...
            '<p class="title" style="padding:2px;display: block; width: 100% !important; text-align: start; font-weight: 600; margin: 0 auto;white-space: pre-wrap;">' + data.options[i].title + '</p>';
            var subtitle = data.options[i].subtitle.split('||');
            for(var i in subtitle)
            {
                carousal += '<p class="subtitle" style="padding:2px; display: block; width: 100% !important; text-align: start; font-weight: 400; margin: 0 auto;word-wrap: break-word;font-size: 13px;white-space: nowrap;">' + subtitle[i] + '</p>';
            }
        $(carousal).appendTo(carousal_container);
    }
    if (data.options.length == 1) {
...
function addCarousalText(data) {
    ...
    for (var i in data.options) {
        var carousal = '<div class="carouselala-inner" id="carousal_' + chatCount + '_option_' +
            '" style=" text-align: justify; width:  calc(100% - 20px); height: auto; overflow: hidden; border: 1px solid #295ba2; border-radius: 5px;">' +
            '<p class="title" style="padding:5px;display: block; width: 100% !important; text-align: start; font-weight: 600; margin: 0 auto;white-space: pre-wrap;">' + data.options[i].title + '</p><div style="background-color: #295ba2; display: block; color: #295ba2; padding: 0; margin: 0; width: 100%; height: 1px;"></div>';
        for (var x in data.options[i].buttons) {
            if (data.options[i].buttons[x].type === 'postback') {
                ...
                carousal += '<button name="' + data.belongs + ',' + 'enable' + '" value="' + data.options[i].buttons[x].payload + '" id="single_button_carousal_' + data.options[i].buttons[x] + '" class="carousel_option" style="display: block; font-size: 13px;word-wrap: break-word; ">' + data.options[i].buttons[x].title + '</button>';
            } else if (data.options[i].buttons[x].type === 'web_url') {
...
function addTyping() {
    ...
    $('<div class="selekt-conversation-part selekt-conversation-part-admin selekt-conversation-part-grouped typing">'
        ...
        + '<div class="selekt-comment"><div class="selekt-blocks"><div class="selekt-block selekt-block-paragraph" style="width: 40px;"><img src="../frontend/assets/typing.gif" style="width: 75%; height:auto;"></div></div></div></div></div>').appendTo($('.selekt-conversation-parts')).show("drop", {
        direction: "down",
        easing: 'easeInOutExpo'
    });
    updateScrollbar();
...
function addSingleSelectionMessage(data) {
    ...
    singleSelectOptions.attr('id', 'chat_' + chatCount);
    for (var i in data.options) {
        console.log("Options size",data.options.length);
